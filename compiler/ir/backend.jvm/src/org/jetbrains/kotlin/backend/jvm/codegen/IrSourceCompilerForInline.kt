...
class IrSourceCompilerForInline(
    ...
    override fun isCallInsideSameModuleAsDeclared(functionDescriptor: FunctionDescriptor): Boolean {
        ...
        val outDirectory = codegen.state.outDirectory
        return function.module == codegen.irFunction.module || function.isContainedByCompiledPartOfOurModule(outDirectory)
    }
...
private fun IrFunction.getBinaryClass(): KotlinJvmBinaryClass? {
    return when (val source = parentAsClass.source) {
        is KotlinJvmBinarySourceElement -> source.binaryClass
        is KotlinJvmBinaryPackageSourceElement ->
            source.getContainingBinaryClass(this) ?: source.getRepresentativeBinaryClass()
        else -> null
    }
}

private fun KotlinJvmBinaryPackageSourceElement.getContainingBinaryClass(function: IrFunction): KotlinJvmBinaryClass? {
    val className = (function.containerSource as? JvmPackagePartSource)?.className ?: return null
    return getContainingBinaryClass(className)
}

private fun IrFunction.isContainedByCompiledPartOfOurModule(friendPath: File?): Boolean {
    if (friendPath == null) return false
    val binaryClass = getBinaryClass() ?: return false
